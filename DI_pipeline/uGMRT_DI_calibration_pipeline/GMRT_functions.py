...
def find_EVLA_band(frequency):
   FLOW = [ 0.0e6, 150.0e6, 155.0e6, 2.0e9, 4.0e9, 8.0e9, 12.0e9, 18.0e9, 26.5e9, 40.0e9 ]
   FHIGH = [ 150.0e6, 155.0e6, 2.0e9, 4.0e9, 8.0e9, 12.0e9, 18.0e9, 26.5e9, 40.0e9, 56.0e9 ]
...
#    if (int(obs_year) < 2010):
    if (int(obs_year) < 2100):
        if (print_offsets):
#            print 'Does not work for VLA observations'
            print 'Does not work for GMRT observations'
        return [1, '', []]
...
def testBPdgains(calMs,calTable,calSpw,calScans,calSolint,refAnt,minBL_for_cal,priorcals,do3C84,UVrange3C84):
    ...
    if (do3C84 == True):
        gaincal(vis=calMs,caltable=calTable,field='',spw=calSpw,intent='',\
            ...
            preavg=-1.0,refant=refAnt,minblperant=minBL_for_cal,minsnr=1.0,\
            solnorm=False,solmode ='L1R',\
...
def testdelays(calMs,calTable,calField,calScans,refAnt,minBL_for_cal,priorcals,do3C84,UVrange3C84,delayspw):
...
#    else:
    ...
    gaincal(vis=calMs,caltable=calTable,field=calField,spw=delayspw,intent='',\
        selectdata=True,uvrange='',scan=calScans,solint='inf',\
        combine='scan',preavg=-1.0,refant=refAnt,\
        minblperant=minBL_for_cal,minsnr=1.0,solnorm=False,\
        gaintype='K',smodel=[],calmode='p',append=False,\
        docallib=False,\
        gaintable=GainTables,gainfield=[''],\
        interp=[''],spwmap=[],parang=False)
...
# delay.k and BPcal.b
    ...
    gaincal(vis=calMs,caltable=calTable,field='',spw=calSpw,intent='',\
        ...
        preavg=-1.0,refant=refAnt,minblperant=minBL_for_cal,minsnr=3.0,\
        solnorm=False,solmode='L1R',\
        gaintype='G',smodel=[],calmode='ap',append=False,\
        docallib=False,\
...
def semiFinaldelays(calMs,calTable,calField,calScans,refAnt,minBL_for_cal,priorcals,do3C84,UVrange3C84,delayspw):
...
def visstatamp(myfile,myspw,myant,mycorr,myscan):
	default(visstat)
