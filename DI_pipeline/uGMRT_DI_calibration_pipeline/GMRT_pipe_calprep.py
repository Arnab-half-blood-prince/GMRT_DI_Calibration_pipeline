...
logprint ("Starting GMRT_pipe_calprep.py", logfileout='logs/calprep.log')
...
flux_set_jy = []  ## This will store the flux density set for flux cal ## 
ii=0
for fields in standard_source_fields:
    for myfield in fields:
        ...
        for myspw in spws:
            ...
            usescratch=scratch
            myset = setjy()
            flux_set_jy.append(myset[str(myfield)]['0']['fluxd'][0])
    ii=ii+1

tb.close()

print "The flux density set by setjy task of Perley-Butler 2017 model",flux_set_jy[0]

logprint("Finished setting models for known calibrators", logfileout='logs/calprep.log')

logprint ("Finished GMRT_pipe_calprep.py", logfileout='logs/calprep.log')
