...
# Version updated 2012-12-17 STM - requires 20121217 lib_EVLApipeutils.py

#!cp /home/sandrock2/smyers/casa/pipeline/pipeline4.1/EVLA_pipe_flag_baddeformatters.py .

logprint("Starting GMRT_pipe_flag_baddeformatters.py", logfileout='logs/flag_baddeformatters.log')
...
logprint("Will test on quantity: "+testq, logfileout='logs/flag_baddeformatters.log')
logprint("Will test using statistic: "+tstat, logfileout='logs/flag_baddeformatters.log')
if testunder:
    logprint("Will flag values under limit = "+str(testlimit), logfileout='logs/flag_baddeformatters.log')
else:
    logprint("Will flag values over limit = "+str(testlimit), logfileout='logs/flag_baddeformatters.log')

logprint("Will identify basebands with more than "+str(nspwlimit)+" bad spw", logfileout='logs/flag_baddeformatters.log')

if doflagundernspwlimit:
    logprint("Will identify individual spw when less than "+str(nspwlimit)+" bad spw", logfileout='logs/flag_baddeformatters.log')

if doflagemptyspws:
    logprint("Will identify spw with no unflagged channels", logfileout='logs/flag_baddeformatters.log')

logprint("Will use flag REASON = "+flagreason, logfileout='logs/flag_baddeformatters.log')

if doflagdata:
    logprint("Will flag data based on what we found", logfileout='logs/flag_baddeformatters.log')
else:
    logprint("Will NOT flag data based on what we found", logfileout='logs/flag_baddeformatters.log')
        ...
        for bband in calBPstatresult['antband'][iant][rrx].keys():
            ...
            if len(spwl)>0:
                if doprintall:
                    ...
                    logprint (strprt, logfileout='logs/flag_baddeformatters.log')
                for ispw in spwl:
                    #
                    testvalid = False
...
if nflagcmds<1:
    logprint ("No bad basebands/spws found", logfileout='logs/flag_baddeformatters.log')
else:
    logprint ("Possible bad basebands/spws found:", logfileout='logs/flag_baddeformatters.log')
    #
    for flagstr in flaglist:
        logprint ("   "+flagstr, logfileout='logs/flag_baddeformatters.log')
    if len(extflaglist)>0:
        logprint ("   ", logfileout='logs/flag_baddeformatters.log')
        for flagstr in extflaglist:
            logprint ("   "+flagstr, logfileout='logs/flag_baddeformatters.log')
        flaglist.extend(extflaglist)
    if doflagdata:
        logprint ("Flagging these in the ms:", logfileout='logs/flag_baddeformatters.log')
...
logprint ("QA2 score: "+QA2_flag_baddeformatters, logfileout='logs/flag_baddeformatters.log')
logprint ("Finished GMRT_pipe_flag_baddeformatters.py", logfileout='logs/flag_baddeformatters.log')
