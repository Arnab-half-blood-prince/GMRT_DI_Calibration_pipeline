...
logprint ("Starting GMRT_pipe_testBPdcals.py", logfileout='logs/testBPdcals.log')
...
if (cal3C84_d == True):
    ...
    spw=tst_delay_spw   ## Leaving edge channels 
    intent=''
    selectdata=True
    uvrange=uvrange3C84
...
flaggedSolnResult=testdelays(ms_active,'testdelay.k',delay_field_select_string,delay_scan_select_string,refAnt,minBL_for_cal,priorcals,cal3C84_d,uvrange3C84,tst_delay_spw)
...
## Here I choose central good channels to do the gaincal by using tst_bpass_spw
soltime=int_time
solint='int'
...
if (cal3C84_bp == True):
    ...
    caltable='testBPcal.b'
    field=bandpass_field_select_string
    spw=tst_delay_spw            #  Neglecting end channels (Arnab during summer 2019). This value is mentioned in GMRT_msinfo.py 
    intent=''
    selectdata=True
    uvrange=''
    scan=bandpass_scan_select_string
    ...
    refant=refAnt
    minblperant=minBL_for_cal
    minsnr=1.0               # minsnr now 1. Earlier it was 5 . (Arnab during summer 2019)
    solnorm=False
    bandtype='B'
    fillgaps=0
    smodel=[]
...
logprint ("Finished GMRT_pipe_testBPdcals.py", logfileout='logs/testBPdcals.log')
