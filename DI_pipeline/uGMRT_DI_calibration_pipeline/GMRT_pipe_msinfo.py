...
logprint ("Starting GMRT_pipe_msinfo.py", logfileout='logs/msinfo.log')
...
channels = tb.getcol('NUM_CHAN')  # this will give total number of channels 
...
elif channels[0] == 2048:
	goodchans = '0:500~600'   # used for visstat
	flagspw = '0:101~1900'
	gainspw = '0:201~1800'
	
elif channels[0] == 4096:
	goodchans = '0:1000~1200'
	flagspw = '0:41~4050'
	gainspw = '0:201~3600'
	
elif channels[0] == 8192:
	goodchans = '0:2000~3000'
	flagspw = '0:500~7800'
	gainspw = '0:1000~7000'
	
elif channels[0] == 16384:
...
print "Number of channels in the ms file:", channels[0]
print "The channel range used to do BP and delay:", flagspw
print "Central good channel used to do gaincal:", gainspw
...
tst_delay_spw=flagspw  ## Leaving edge channels to do delay cal 
all_spw=''
...
tst_bpass_spw = gainspw  # changed this to get gain sol for central channels 
...
# GMRT flux cal
        flux_scan_select_string = gmrt_flux_scan
        flux_field_select_string = gmrt_flux_field
        flux_scan_list = [int(x) for x in flux_scan_select_string.split(',') if x]
        flux_field_list = [int(x) for x in flux_field_select_string.split(',') if x] 
...
# GMRT Bandpass cal
        bandpass_scan_select_string = gmrt_bandpass_scan
        bandpass_field_select_string = gmrt_bandpass_field
        bandpass_scan_list = [int(x) for x in bandpass_scan_select_string.split(',') if x]
        bandpass_field_list = [int(x) for x in bandpass_field_select_string.split(',') if x]
        logprint ("Bandpass calibrator(s) scans are "+bandpass_scan_select_string, logfileout='logs/msinfo.log')
        logprint ("Bandpass calibrator(s) are fields "+bandpass_field_select_string, logfileout='logs/msinfo.log')
...
# GMRT Delay cal
        delay_scan_select_string = gmrt_delay_scan
        delay_field_select_string = gmrt_delay_field
        delay_scan_list = [int(x) for x in delay_scan_select_string.split(',') if x]
        delay_field_list = [int(x) for x in delay_field_select_string.split(',') if x]
        logprint ("Delay calibrator(s) scans are "+delay_scan_select_string, logfileout='logs/msinfo.log')
        logprint ("Delay calibrator(s) are fields "+delay_field_select_string, logfileout='logs/msinfo.log')
...
# GMRT phase cal
        phase_scan_select_string = gmrt_phase_scan
        phase_field_select_string = gmrt_phase_field
        phase_scan_list = [int(x) for x in phase_scan_select_string.split(',') if x]
        phase_field_list = [int(x) for x in phase_field_select_string.split(',') if x]
...
#  
# GMRT all cal 
    calibrator_scan_select_string = gmrt_calibrator_scan
    calibrator_field_select_string = gmrt_calibrator_field
    calibrator_scan_list = [int(x) for x in calibrator_scan_select_string.split(',') if x]
    calibrator_field_list = [int(x) for x in calibrator_field_select_string.split(',') if x]
#
...
minBL_for_cal=max(3,int(numAntenna/8.0))  # Number of max baseline is now 4 for GMRT for  Initially it was set to numAntenna/2.0 . So it was 16 before. [Arnab during summer 2019]
...
syscommand='rm -rf plotantslog.png'
os.system(syscommand)
...
logprint ("Antenna positions plot (logarithmic) plotantslog.png", logfileout='logs/msinfo.log')
...
logprint ("Finished GMRT_pipe_msinfo.py", logfileout='logs/msinfo.log')
