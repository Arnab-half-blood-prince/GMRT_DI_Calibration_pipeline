...
# CHECKING FLAGGING OF ALL CALIBRATORS after 2nd round of semifinalbpdelay  
# use rflag mode of flagdata

logprint ("Starting GMRT_pipe_checkflag_semiFinal2.py", logfileout='logs/checkflag_semiFinal2.log')
time_list=runtiming('checkflag_semiFinal2', 'start')
QA2_checkflag_semiFinal='Pass'

logprint ("Checking RFI flagging of all calibrators", logfileout='logs/checkflag_semiFinal2.log')
...
for ii in range(len(myampcals)):
    ...
    flagdata(vis=ms_active,mode="tfcrop", datacolumn="corrected", field=myampcals[ii], ntime="120s",
        		timecutoff=6.0, freqcutoff=6.0, timefit="line",freqfit="line",flagdimension="freqtime", 
        		extendflags=False, usewindowstats='both',halfwin=2, extendpols=False,growaround=False,
        		action="apply", flagbackup=True,overwrite=True, writeflags=True)

    flagdata(vis=ms_active,mode="rflag",datacolumn="corrected",field=myampcals[ii], timecutoff=6.0, 
...
for ii in range(len(mypcals)):
    ...
    flagdata(vis=ms_active,mode="clip",spw='',field=mypcals[ii], clipminmax=[0,1.5*mean],
        		datacolumn="corrected",clipoutside=True, clipzeros=True, extendpols=False, 
        		action="apply",flagbackup=True, savepars=False, overwrite=True, writeflags=True)
    ...
    flagdata(vis=ms_active,mode="rflag",datacolumn="corrected",field=mypcals[ii], timecutoff=6.0, 
		        freqcutoff=6.0,timefit="poly",freqfit="line",flagdimension="freqtime", extendflags=False,
		        timedevscale=4.0,freqdevscale=4.0,spectralmax=500.0,extendpols=False, growaround=False,
		        flagneartime=False,flagnearfreq=False,action="apply",flagbackup=True,overwrite=True, writeflags=True)
...
afterflags = flagdata()
#clearstat()
logprint ("final flags summary", logfileout='logs/checkflag_semiFinal2.log')

after_total = afterflags['total']
after_flagged = afterflags['flagged']
logprint ("After flagging the flagged fraction = "+str(after_flagged/after_total), logfileout='logs/checkflag_semiFinal2.log')


logprint ("QA2 score: "+QA2_checkflag_semiFinal, logfileout='logs/checkflag_semiFinal2.log')
logprint ("Finished GMRT_pipe_checkflag_semiFinal2.py", logfileout='logs/checkflag_semiFinal2.log')
time_list=runtiming('checkflag_semiFinal2', 'end')
